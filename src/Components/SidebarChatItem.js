...
export const SidebarChatItem = ({ usuario }) => {
  ...
  return (
    <div
      className={`chat_list  ${usuario.uid === chatActivo && "active_chat"}`} //Resaltar el chat seleccionado.
      onClick={activarChat}
    >
      {/*active_chat*/}
      <div className="chat_people">
        ...
        <div className="chat_ib">
          <h5> {usuario.nombre} </h5>
          {
            ...
            ) //Si esta online = false, le pone offline
          }
        </div>
        <div className="mensaje-entrante">
          {mensajesNuevos
            ? mensajesNuevos.map((msg) => {
                if (nuevo && usuario && msg && chatState) {
                  if (
                    nuevo &&
                    usuario.uid === msg.de &&
                    !mismoUsuario &&
                    chatState.chatActivo !== msg.de
                  ) {
                    mismoUsuario = true;

                    return (
                      <span key={msg.id} className="nuevo-msg">
                        Nuevo mensaje
                      </span>
                    );
                  }
